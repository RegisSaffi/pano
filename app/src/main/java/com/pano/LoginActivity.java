...
public class LoginActivity extends AppCompatActivity {
    ...
    TextView time;
    Pinview etOTP;
    FancyButton btnSignIn, btnLogin, google, facebook;
    Button resend;
    FirebaseAuth auth;

    FacebookAuthProvider mCallbackManager;
...
//        Crashlytics.getInstance().crash();
        ...
        btnLogin.setOnClickListener(v -> {
            ...
            if (!txtPhone.getText().toString().trim().equals("")) {
                ...
                if (phne.startsWith("0")) {
                    phne = phne.substring(1);
                }

                if (!phne.startsWith("+")) {
                    phoneNumber = "+" + ccp.getSelectedCountryCode() + phne;
                } else {
                    phoneNumber = phne;
                }
    ...
    private void customTextView(TextView view) {
        SpannableStringBuilder spanTxt = new SpannableStringBuilder(
                "By registering with this app you agree with the ");
        spanTxt.append("Term and conditions");
        spanTxt.setSpan(new ClickableSpan() {
            @Override
            public void onClick(View widget) {
//                Toasty.error(getApplicationContext(), "Terms of services Clicked",
//                        Toasty.LENGTH_SHORT).show();

                Intent in = new Intent(Intent.ACTION_VIEW);
                in.setData(Uri.parse("http://www.pano.rw/terms.php"));
                startActivity(in);

            }
        }, spanTxt.length() - "Term and conditions".length(), spanTxt.length(), 0);
        spanTxt.append(" of PANO");
...
//
    ...
    public void loginAccount() {
        phoneNumber = phoneNumber.replace("+", "").replaceAll(" ", "");
